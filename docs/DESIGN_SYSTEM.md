...
Simulated status-bar and home-indicator layers use `--ios-system-foreground`: black over light content and white over dark content. Both layers have transparent backgrounds, including with the keyboard open; the app/map/keyboard surface continues beneath them. They are siblings of the app owner, independent of brand text and header blur. The app owner isolates overlay stacking below system chrome. Simulated chrome stays hidden on unframed mobile layouts.
...
The map fills the entire screen behind transparent, independent status-bar and home-indicator layers. Their glyphs retain neutral light/dark system colors. Only map controls and attribution consume the screen safe-area insets, keeping them clear of the system chrome. The outer frame remains 402 × 874, with the circular mouse cursor and no-hover controls. Map appearance follows the shared theme; zoom, north reset, recenter and point selection reuse the existing implementation. The blue location marker remains a simulated Grand Central position. Shared canvas and marker styles now live with LiveMap in studio/live-map.css, also used by the original mockup. The existing public Mapbox browser token is used by the local preview; no secret credentials were copied.

### Map actions and chat entry

The map uses the existing Ionic iOS controls and neutral system surface tokens for a round menu button at top left, search at top right, and a bottom message composer. The composer has attachment and microphone actions; nonempty text replaces the mic with Send. All targets remain at least 44px, with no hover effects inside the phone. Map controls and attribution clear the measured composer/context height. Status and home layers remain transparent.

The in-phone menu offers New chat, Conversation and Find a place. Search filters the existing three Grand Central sample locations and centers the map on selection; it is not live geocoding. Attachments use the existing upload-metadata helper, stay local, and are not uploaded or analyzed. Messages remain in memory for this preview session. Send opens a full-screen conversation with clearly labeled sample answers reused from the original prototype; a live AI service is not connected. Browser speech recognition starts only on a mic tap, supports stopping, and reports unavailable/blocked/error states. Panels stay inside the phone, isolate the map with inert, trap keyboard focus, and restore it on Escape or close.

### Phone focus appearance

By explicit prototype preference, phone controls have no visual focus state: no outlines, rings, focus-only shadows or Ionic focus overlays. Keyboard focus, Tab navigation, text carets, typing, modal focus trapping/restoration, and pressed/selected/disabled states remain functional. Desktop workspace controls retain visible focus styling. This is a scoped prototype appearance rule, not a claim that iOS has no focus system. The installed ios-web-foundation skill and runtime reference carry the same rule and default transparent status/home layers.

### Message keyboard

Focusing or clicking the map message field opens the existing iOS 18 keyboard simulation on fine-pointer desktops. The composer and map controls clear the measured keyboard height; the fixed phone frame never shrinks. Keys edit at the caret, respect selections and the 500-character limit, delete complete Unicode code points, and Send submits the draft. Escape, tapping outside the composer/keyboard, opening a panel, starting voice, or choosing an attachment dismisses the preview keyboard. Coarse-pointer devices retain their native keyboard. The keyboard surface extends beneath the transparent home-indicator layer without a separate system-bar background. No Hide keyboard control or visible focus treatment is added.

### App menu drawer

The in-phone menu is a left-edge drawer, 304px wide on the standard preview and limited to leave at least 56px of dimmed map visible at narrower widths. It fills the inner screen height beneath transparent system chrome, with safe-area padding and left-aligned actions. Enter/exit motion slides horizontally; reduced motion removes the animation. Close, backdrop and Escape dismiss it; the map remains inert through exit and focus returns afterward. New chat focuses the composer after closing. The desktop workspace drawer remains separate. The installed ios-web-foundation skill now requires this menu presentation instead of an inset desktop-style modal card.

### Full-screen conversation

Send opens an app page filling the phone screen, rather than a modal. The page has persistent top controls: the app menu on the left, and New chat plus a three-dot options button on the right. There is no centered title or subtitle header. The transcript scrolls below these controls, with right-aligned user messages, sourced sample answers, and the shared attachment/message/microphone composer and keyboard. New chat starts an empty conversation and focuses the composer while retaining earlier conversations for the open preview session. The three-dot control opens a contained bottom action sheet with View map and Find a place. The app drawer contains Places (the map), Routes (currently an empty state), and a Recent section listing actual conversations by first-message title, ordered by their latest sent message. Reopening a recent conversation restores its messages and attachment context. A brief replying state precedes each deterministic sample answer. The sample-answer helper is shared with the original Studio mockup. Place context carries into follow-ups; selecting a sample place by name in the initial question also establishes context. Attachments and unknown places receive explicit limitation responses. Back and browser history return to the map; the Recent section reopens in-memory conversations.

Sent attachments appear as cards inside their user message, above the message text. Place cards show a location icon and name; uploaded files show their name, type and size. Sending clears the pending composer attachment while preserving a snapshot on the message. Follow-up answers retain the latest conversation context without repeating its card on every message.
