...
const themeStorageKey = 'placewise.studio.theme'

function readThemePreference(): ThemePreference {
  try {
    const saved = localStorage.getItem(themeStorageKey)
    if (saved === 'light' || saved === 'dark' || saved === 'system') return saved
  } catch { /* Theme switching stays available when browser storage is blocked. */ }
  return 'system'
}
...
export default function Studio() {
  ...
  useEffect(() => {
    ...
    } catch { /* Browsers without a color-scheme API keep the light fallback. */ }
  }, [])
  ...
  useLayoutEffect(() => {
    ...
    try { localStorage.setItem(themeStorageKey, themePreference) } catch { /* Session-only fallback. */ }
  }, [resolvedMode, themePreference])
